...
traindataimages = trdata.flow_from_directory(directory="/home-1/aeshak1@jhu.edu/scratch/ccmlo2/cc/Training", target_size=(5500, 5500), batch_size=4,color_mode='grayscale')
...
testdataimages = trdata.flow_from_directory(directory="/home-1/aeshak1@jhu.edu/scratch/ccmlo2/cc/Test", target_size=(5500, 5500), batch_size=4,color_mode='grayscale')
...
with central_storage_strategy.scope():
        ...
        checkpoint = ModelCheckpoint("ccmodel_v5_minimized.hdf5", monitor='loss', verbose=1, save_best_only=True, mode='auto', period=1)
        # Compile the model
        model_0.compile(optimizer='Nadam', loss='binary_crossentropy', metrics=['accuracy'])

        model_results = model_0.fit(traindataimages, steps_per_epoch=250, validation_steps=100, batch_size=4, epochs=30, validation_data=testdataimages, validation_batch_size=4, callbacks=[checkpoint])
