...
from tensorflow.keras.layers import Model,Dense, Dropout, Activation, Flatten
...
traindataimages = trdata.flow_from_directory(directory="/home-1/aeshak1@jhu.edu/scratch/full-resolution/training/images", target_size=(2900,4000), batch_size=1,color_mode='grayscale')
traindatamasks = trdata.flow_from_directory(directory="/home-1/aeshak1@jhu.edu/scratch/full-resolution/training/masks", target_size=(2900,4000), batch_size=1,color_mode='grayscale')
traindata = zip(traindataimages,traindatamasks)
...
with central_storage_strategy.scope():
        ...
        checkpoint = ModelCheckpoint("best_model4.hdf5", monitor='accuracy', verbose=1, save_best_only=True, mode='auto', period=1)
        # Compile the model
        model_0.compile(optimizer='Nadam', loss='binary_crossentropy', metrics=['accuracy'])

        model_0.fit(x=traindataimages, y=traindatamasks,  batch_size=1, steps_per_epoch=200, epochs=250, callbacks=[checkpoint])
#
