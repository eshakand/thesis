...
traindata = trdata.flow_from_directory(directory="/home-1/aeshak1@jhu.edu/scratch/roi_training/training", target_size=(224,224), batch_size=1,color_mode='grayscale')
...
with central_storage_strategy.scope():
        ...
        model_0.add(Conv2D(filters=64, kernel_size=(3, 3), padding='same', strides=(1,1), activation='relu', input_shape=(224,224,1)))
        ...
        checkpoint = ModelCheckpoint("vgg19.hdf5", monitor='accuracy', verbose=1, save_best_only=True, mode='auto', period=1)
        # Compile the model
        model_0.compile(optimizer='Nadam', loss='binary_crossentropy', metrics=['accuracy'])

        model_0.fit(traindata, batch_size=1, epochs=250, steps_per_epoch=200, callbacks=[checkpoint])
#
